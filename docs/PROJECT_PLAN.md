...
Issue #15 已在真实 GitHub runner 与 `cloudflare-production` environment 完成远端证据。Issue #16 将同一 Worker 的 `/GameYard/` 设为第二个公开 mount；Worker 入口与 `dist` 一起进入唯一 artifact，两个 URL 不产生第二次构建。当前唯一正式 origin 为 `https://gameyard.lepus.icu`，生产 Worker 不发布 `workers.dev` 入口。
