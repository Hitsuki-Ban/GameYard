...
function createStaticServer() {
  ...
  return createServer((request, response) => {
    ...
    if (
      (realTarget !== REAL_TESTKIT_ROOT && !realTarget.startsWith(realRootPrefix)) ||
      !file.isFile()
    ) {
      response.writeHead(403);
      response.end("Forbidden");
    ...
    response.writeHead(200, {
      ...
      "Content-Type":
        MIME_TYPES.get(path.extname(realTarget).toLowerCase()) || "application/octet-stream",
    });
...
async function moveCursor(page, target) {
  ...
  for (let count = Math.abs(target.x - cursor.x); count > 0; count--)
    await page.keyboard.press(horizontal);
  for (let count = Math.abs(target.y - cursor.y); count > 0; count--)
    await page.keyboard.press(vertical);
}

async function applyLocale(page, preference, resolved) {
  await page.evaluate((locale) => globalThis.__CB_HOST__.send("locale.apply", { locale }), {
    preference,
    resolved,
  });
  await page.waitForFunction(
    (value) => document.documentElement.lang === value,
    resolved === "zh-Hans" ? "zh-CN" : resolved,
  );
}
...
async function runJourney(page) {
  ...
  assert.match(
    await page.getAttribute("#game-canvas", "aria-keyshortcuts"),
    /ArrowUp.*Enter.*Space.*Escape/,
  );
  ...
  assert.equal(
    (await state(page)).selectedId,
    null,
    "Escape must cancel selection before requesting pause.",
  );
  ...
  await page.waitForFunction(
    () => !globalThis.__CB_TEST__.state().paused && document.activeElement?.id === "game-canvas",
  );
  ...
  await page.waitForFunction(
    () =>
      globalThis.__CB_TEST__.state().mode === "run" && document.activeElement?.id === "game-canvas",
  );
  ...
  await page.waitForFunction(
    () =>
      globalThis.__CB_TEST__.state().phase === "player" &&
      document.activeElement?.id === "game-canvas",
  );

  await page.evaluate(() => {
    globalThis.__CB_TEST__.fast(true);
    if (!globalThis.__CB_TEST__.finishBattle())
      throw new Error("The first run battle could not be completed.");
  });
  await page.waitForFunction(
    () => globalThis.__CB_TEST__.state().activeModal === "reward-modal",
    null,
    { timeout: 5000 },
  );
  ...
  await page.evaluate(() => {
    ...
    globalThis.__CB_TEST__.fast(true);
    if (!globalThis.__CB_TEST__.finishBattle())
      throw new Error("The final run battle could not be completed.");
  });
  await page.waitForFunction(
    () => globalThis.__CB_TEST__.state().activeModal === "run-result-modal",
    null,
    { timeout: 5000 },
  );
