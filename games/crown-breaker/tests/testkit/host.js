...
const context = {
  ...
  instanceId: "crown-breaker-testkit",
  baseUrl: "./crown-breaker/",
...
function fail(message) {
  ...
  host.failed = true;
  const error = new Error(`CrownBreaker testkit Host failed: ${message}`);
  console.error(error);
...
function send(type, fields = {}) {
  if (!host.port || host.failed)
    return Promise.reject(new Error("CrownBreaker testkit Host is not connected."));
...
window.addEventListener("message", (event) => {
  ...
  if (
    ...
    event.data.gameId !== "crown-breaker" ||
  ...
  host.port.addEventListener("messageerror", () => fail("MessagePort decoding failed"));
  host.port.start();
