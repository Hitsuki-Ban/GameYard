(() => {
  ...
  const LOCALE_MODES = Object.freeze(['auto', ...SUPPORTED_LOCALES]);
  ...
  const CATALOGS = Object.freeze({
    'zh-CN': Object.freeze({
      ...
      'aria.gameControls': '键盘操作：方向键或 A、D 移动；向下键或 S 软降；向上键、X 或 K 顺时针旋转；Z 或 J 逆时针旋转；空格快速落下；C 或 L 攻击；V 或分号防御；Escape 或 P 暂停。攻击和防御需要至少 100 能量。',
      'aria.attackControl': '攻击。快捷键 C 或 L；至少需要 100 能量。',
      'aria.defenseControl': '防御。快捷键 V 或分号；至少需要 100 能量。',
      'aria.gameSummary': '你：分数 {playerScore}，能量 {playerEnergy}，盘面危险度 {playerDanger}%，来袭 {playerIncoming} 行。对手：分数 {cpuScore}，能量 {cpuEnergy}，盘面危险度 {cpuDanger}%，来袭 {cpuIncoming} 行。',
    ...
    ja: Object.freeze({
      'meta.description': '中国語・日本語・英語に対応した、オリジナルの2面対戦型落ちものWebゲーム。',
      ...
      'title.wins': '勝',
      'title.bestChain': '連鎖',
      'title.footnote': 'オリジナル WEB ゲーム · オフライン対応',
      'hud.pause': 'ポーズ',
      'touch.label': 'タッチ操作',
      'touch.left': '左へ移動',
      ...
      'help.centerBody': '中央の出現列が上端に達したときだけ敗北します。両側は壁として活用できます。',
      'help.reverseTitle': '逆転連鎖',
      'help.reverseBody': '攻撃で増える列も通常ブロックです。相手の消去を助けることがあり、攻撃は常に安全とは限りません。',
      'controls.move': '移動',
      ...
      'aria.gameControls': 'キーボード操作：矢印キーまたは A・D で移動、下矢印または S でソフトドロップ、上矢印・X・K で右回転、Z または J で左回転、スペースでハードドロップ、C または L で攻撃、V またはセミコロンで防御、Escape または P でポーズします。攻撃と防御には100以上のエナジーが必要です。',
      'aria.attackControl': '攻撃。ショートカットは C または L。100以上のエナジーが必要です。',
      'aria.defenseControl': '防御。ショートカットは V またはセミコロン。100以上のエナジーが必要です。',
      'aria.gameSummary': '自分：スコア {playerScore}、エナジー {playerEnergy}、盤面危険度 {playerDanger}%、接近中 {playerIncoming} 行。相手：スコア {cpuScore}、エナジー {cpuEnergy}、盤面危険度 {cpuDanger}%、接近中 {cpuIncoming} 行。',
    ...
    en: Object.freeze({
      'meta.description': 'An original split-board competitive falling-block web game in Chinese, Japanese, and English.',
      ...
      'title.footnote': 'ORIGINAL WEB GAME · OFFLINE READY',
      'hud.pause': 'Pause',
      ...
      'help.connectBody': 'Connect three matching colors vertically or horizontally to clear them. Continued clears form chains.',
      ...
      'help.attackBody': 'At 100 energy or more, spend everything to push colored rows up from your opponent’s floor.',
      'help.defenseTitle': 'Full-power defense',
      'help.defenseBody': 'Cancel incoming attacks and clear high-risk blocks near the top of your board.',
      'help.centerTitle': 'Protect the center',
      'help.centerBody': 'You lose only when the center spawn column reaches the top. Use the sides as deliberate walls.',
      'help.reverseTitle': 'Reverse chains',
      'help.reverseBody': 'Attack rows are normal colored blocks and may complete an opponent’s clear, so every attack is a calculated risk.',
      'controls.move': 'MOVE',
      ...
      'aria.gameControls': 'Keyboard controls: move with the arrow keys or A and D; soft drop with Down or S; rotate clockwise with Up, X, or K; rotate counterclockwise with Z or J; hard drop with Space; attack with C or L; defend with V or semicolon; and pause with Escape or P. Attack and defense require at least 100 energy.',
      ...
      'aria.gameSummary': 'You: score {playerScore}, energy {playerEnergy}, board danger {playerDanger} percent, {playerIncoming} incoming lines. Opponent: score {cpuScore}, energy {cpuEnergy}, board danger {cpuDanger} percent, {cpuIncoming} incoming lines.',
  ...
  const placeholders = value => [...value.matchAll(/\{([a-z][a-zA-Z0-9]*)\}/g)].map(match => match[1]).sort();
  ...
  function assertMode(mode) {
    if (!LOCALE_MODES.includes(mode)) throw new RangeError(`Unsupported locale mode: ${mode}`);
    return mode;
  }

  class I18n {
    constructor({ mode = 'auto', languages } = {}) {
      this.mode = assertMode(mode);
      this.locale = this.mode === 'auto' ? I18n.resolveSystemLocale(languages) : this.mode;
      this.listeners = new Set();
      this.numberFormatter = new Intl.NumberFormat(this.locale);
      this.boundLanguageChange = () => this.refreshSystemLocale();
      window.addEventListener('languagechange', this.boundLanguageChange);
    }

    static resolveSystemLocale(languages) {
      const preferences = languages === undefined
        ? (navigator.languages?.length ? navigator.languages : [navigator.language])
        : (Array.isArray(languages) ? languages : [languages]);
      for (const preference of preferences) {
        const language = String(preference || '').trim().toLowerCase().split(/[-_]/)[0];
    ...
    static readStoredMode(storage, key = PLO.CONFIG?.STORAGE_KEY) {
      if (!key) throw new Error('A storage key is required before locale bootstrap.');
      let raw;
      try { raw = (storage || window.localStorage).getItem(key); }
      catch (error) {
        PLO.localeBootstrapIssue = 'unavailable';
        return 'auto';
      }
      ...
      try { parsed = JSON.parse(raw); }
        ...
        PLO.localeBootstrapIssue = 'corrupt';
        return 'auto';
      }
      if (typeof PLO.validateSaveData !== 'function') throw new Error('Save validation must load before locale bootstrap.');
      if (!PLO.validateSaveData(parsed)) {
        PLO.localeBootstrapIssue = 'corrupt';
        return 'auto';
      }
      return parsed.settings.localeMode;
    }

    static get supportedLocales() { return SUPPORTED_LOCALES; }
    static get localeModes() { return LOCALE_MODES; }
    static get catalogKeys() { return CATALOG_KEYS; }

    setMode(mode, languages) {
      const nextMode = assertMode(mode);
      const nextLocale = nextMode === 'auto' ? I18n.resolveSystemLocale(languages) : nextMode;
      if (nextMode === this.mode && nextLocale === this.locale) return false;
      this.mode = nextMode;
      this.setLocale(nextLocale);
      return true;
    }

    refreshSystemLocale(languages) {
      if (this.mode !== 'auto') return false;
      const nextLocale = I18n.resolveSystemLocale(languages);
    ...
    setLocale(locale) {
      ...
      for (const listener of [...this.listeners]) listener({ mode: this.mode, locale });
    }
    ...
    t(key, params = {}) {
      ...
      if (value === undefined) throw new ReferenceError(`Missing translation: ${this.locale}:${key}`);
    ...
    formatNumber(value) {
      ...
      if (!Number.isFinite(number)) throw new TypeError(`Cannot format non-finite number: ${value}`);
      return this.numberFormatter.format(Math.max(0, Math.floor(number)));
    }

    manifestFilename() {
      const filename = MANIFESTS[this.locale];
      if (!filename) throw new ReferenceError(`Missing manifest for ${this.locale}.`);
      return filename;
    }
    ...
    applyHead() {
      ...
      if (!window.__PULSE_LINK_STANDALONE__ && (location.protocol === 'http:' || location.protocol === 'https:')) {
        if (!manifest) {
        ...
        manifest.setAttribute('href', this.manifestFilename());
