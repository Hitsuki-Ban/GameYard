(() => {
  ...
  const CONFIG = Object.freeze({
    ...
    REGULAR_CP: 2,
    PULSE_CP: 100,
    STORAGE_KEY: "gameyard.game.pulse-link-overdrive.save.v1",
  ...
  class RNG {
    ...
    fork(salt = 0) {
      return new RNG((this.nextUint() ^ (salt >>> 0) ^ 0x85ebca6b) >>> 0);
    }
  }
    ...
    emit(type, payload = {}) {
      ...
      for (const handler of handlers) {
        try {
          ...
          console.error(`[PLO event:${type}]`, err);
        }
      }
    }
    clear() {
  ...
  const DEFAULT_SAVE = Object.freeze({
    version: 1,
    settings: Object.freeze({ glyphs: true, haptics: true }),
    stats: Object.freeze({
      wins: 0,
      losses: 0,
      matches: 0,
  ...
  const isWholeRange = (value, min, max = Number.MAX_SAFE_INTEGER) =>
  ...
  const validateSaveData = (value) => {
    if (
      ...
      return false;
    if (value.version !== DEFAULT_SAVE.version || typeof value.tutorialComplete !== "boolean")
      return false;
    if (
      !["duel", "blitz", "lab"].includes(value.selectedMode) ||
      !isWholeRange(value.difficulty, 0, 3)
    )
      return false;
    const settings = value.settings;
    if (!hasExactKeys(settings, ["glyphs", "haptics"])) return false;
    if (![settings.glyphs, settings.haptics].every((item) => typeof item === "boolean"))
      return false;
  ...
  class SaveStore {
    ...
    get storageTarget() {
      return this.storage === undefined ? window.localStorage : this.storage;
    }
    ...
    subscribeStatus(listener) {
      if (typeof listener !== "function")
        throw new TypeError("Save status listener must be a function.");
    ...
    load() {
      ...
      try {
        ...
        if (!validateSaveData(parsed))
          throw new TypeError("Save data does not match the current schema.");
        return parsed;
      } catch (err) {
        ...
        try {
          ...
          this.setRecovery("unavailable");
          console.warn(
            "Invalid local save could not be repaired because storage is unavailable.",
            storageError,
          );
        }
        return repaired;
    ...
    save() {
      ...
      try {
        this.storageTarget.setItem(this.key, JSON.stringify(this.data));
      } catch (err) {
        this.available = false;
        this.setRecovery("unavailable");
