(() => {
  ...
  function simulateResolution(sourceGrid, maxChains = 12) {
    ...
    let chain = 0, regular = 0, pulse = 0, cleared = 0;
  ...
  function groupPotential(grid) {
    const visited = Array.from({ length: CONFIG.ROWS }, () => Array(CONFIG.COLS).fill(false));
    let pairs = 0, adjacency = 0, pulseEdges = 0;
    for (let y = 0; y < CONFIG.ROWS; y++) {
      for (let x = 0; x < CONFIG.COLS; x++) {
        const cell = grid[y][x];
        if (!cell) continue;
        ...
        const queue=[[x,y]]; visited[y][x]=true; let size=0;
        while(queue.length){
          ...
          for(const [dx,dy] of DIRS){
            ...
            if(!inside(nx,ny)||visited[ny][nx]) continue;
            const n=grid[ny][nx];
            if(n?.kind==='normal'&&n.color===cell.color){visited[ny][nx]=true;queue.push([nx,ny]);}
          }
        }
        if (size === 2) pairs++;
  ...
  function gridMetrics(grid) {
    const heights = [];
    let holes = 0, roughness = 0, hidden = 0, total = 0;
    for (let x = 0; x < CONFIG.COLS; x++) {
      let top = CONFIG.ROWS;
      let seen = false;
      for (let y = 0; y < CONFIG.ROWS; y++) {
  ...
  class PieceStream {
    ...
    setScript(pairs) { this.script = pairs.map(pair => pair.map(c => ({ ...c }))); }
    ...
    nextColor() {
      ...
      const weights = Array.from({ length: CONFIG.COLORS }, (_, c) => {
        const droughtBoost = 1 + Math.min(2.2, this.drought[c] * .16);
        const recentPenalty = 1 / (1 + recentCounts[c] * .75);
        const repeatPenalty = this.history.at(-1) === c && this.history.at(-2) === c ? .28 : 1;
        return droughtBoost * recentPenalty * repeatPenalty;
      });
      const color = this.rng.weighted(weights);
      this.history.push(color);
      if (this.history.length > 24) this.history.shift();
      for (let i = 0; i < CONFIG.COLORS; i++) this.drought[i] = i === color ? 0 : this.drought[i] + 1;
      return color;
    }

    nextPair() {
      if (this.script.length) return this.script.shift().map(c => ({ ...c }));
      const chance = Math.min(CONFIG.PULSE_MAX_CHANCE,
        CONFIG.PULSE_BASE_CHANCE + Math.max(0, this.pulsePity - CONFIG.PULSE_PITY_START) * CONFIG.PULSE_PITY_STEP);
      const hasPulse = this.rng.chance(chance);
      if (hasPulse) this.pulsePity = 0;
      else this.pulsePity++;
      const pulseIndex = hasPulse ? this.rng.int(0, 1) : -1;
      return [0, 1].map(i => i === pulseIndex ? { kind: 'pulse', color: -1 } : { kind: 'normal', color: this.nextColor() });
    }
  }
  ...
  class Board {
    ...
    seedTutorial() {
      ...
      this.stream.setScript([
        [{ kind: 'normal', color: 1 }, { kind: 'normal', color: 0 }],
        [{ kind: 'normal', color: 0 }, { kind: 'normal', color: 0 }],
        [{ kind: 'normal', color: 2 }, { kind: 'normal', color: 0 }],
        [{ kind: 'normal', color: 3 }, { kind: 'normal', color: 3 }]
      ]);
      this.nextQueue = [];
      this.ensureQueue();
    }
    ...
    spawn() {
      ...
      this.active = {
        a: { ...pair[0] }, b: { ...pair[1] },
        x: CONFIG.CENTER_COL, y: CONFIG.HIDDEN_ROWS, rot: 2,
        gravity: 0, lock: 0, lockResets: 0, id: ++this.sequence,
        landingPulse: 0, squish: 0
      };
    ...
    move(dx, dy, silent = false) {
      ...
      this.active.x = nx; this.active.y = ny;
      if (dx && this.active.lockResets < 12) { this.active.lock = 0; this.active.lockResets++; }
    ...
    rotate(direction = 1) {
      ...
      for (const [kx, ky] of kicks) {
        if (!this.collides(this.active.x + kx, this.active.y + ky, targetRot)) {
          ...
          this.active.x += kx; this.active.y += ky;
          if (this.active.lockResets < 12) { this.active.lock = 0; this.active.lockResets++; }
    ...
    hardDrop(assist = false) {
      ...
      this.game.bus.emit('hardDrop', { player: this.owner, distance, positions: this.activePositions() });
      this.lockPiece();
      return true;
    }
    ...
    lockPiece() {
      ...
      for (const pos of positions) {
        ...
        this.grid[pos.y][pos.x] = makeCell(pos.cell.kind, pos.cell.color, { id: globalCellId++, wobble: this.rng.next() * Math.PI * 2 });
      }
    ...
    startResolution(counter = false) {
      ...
      this.resolution = {
        phase: 'scan', timer: 0, total: 0, chain: 0,
        counter: !!counter, clear: null, moved: 0
      };
    ...
    advanceResolution() {
      ...
      this.game.bus.emit('clearStart', { player: this.owner, board: this, clear, chain: r.chain, counter: r.counter });
    }
    ...
    performClear() {
      ...
      this.game.bus.emit('clearResolved', { player: this.owner, board: this, clear, chain: r.chain, counter: r.counter });
    ...
    finishResolution() {
      ...
      this.game.bus.emit('resolutionComplete', { player: this.owner, board: this, chain: r.chain, counter: r.counter });
      this.resolution = null;
      for (const row of this.grid) for (const cell of row) if (cell) { cell.fallFrom = null; cell.fallDistance = 0; cell.flash = 0; }
    ...
    update(dt, softDrop = false) {
      ...
      if (this.resolution) {
        ...
        if (r.phase === 'flash' && r.clear) {
          ...
          if (r.timer <= 0) {
            for (const row of this.grid) for (const cell of row) if (cell) { cell.fallFrom = null; cell.fallDistance = 0; }
            r.phase = 'scan'; r.clear = null;
            this.advanceResolution();
          }
        }
        return;
    ...
    gravityInterval(softDrop) {
      ...
      const modeRamp = this.game.mode === 'blitz' ? .84 : 1;
      const ramp = Math.pow(.965, elapsed / 18);
    ...
    makeRiseRow() {
      ...
      for (let x = 0; x < CONFIG.COLS; x++) {
        ...
        if (above?.kind === 'normal') weights[above.color] *= .82;
        const color = this.rng.weighted(weights);
        row.push(makeCell('normal', color, { id: globalCellId++, wobble: this.rng.next() * Math.PI * 2 }));
      }
      return row;
    }
    ...
    applyRiseLines(lines, counterSource = true) {
      ...
      for (let n = 0; n < lines; n++) {
        for (let x = 0; x < CONFIG.COLS; x++) if (this.grid[0][x]) discarded.push({ x, cell: this.grid[0][x] });
      ...
      this.shake = Math.min(1, this.shake + .25 + lines * .1);
    ...
    purgeCells(count, fromQueue = false) {
      ...
      while (remaining > 0) {
        ...
        for (const x of order) {
          let target = -1;
          for (let y = 0; y < CONFIG.ROWS; y++) {
            if (this.grid[y][x]) { target = y; break; }
          }
          if (target >= 0) {
    ...
    dangerRatio() {
      let top = CONFIG.ROWS;
      for (let y = 0; y < CONFIG.ROWS; y++) if (this.grid[y][CONFIG.CENTER_COL]) { top = y; break; }
  ...
  class Player {
    ...
    reset() {
      this.cp = 0; this.displayCp = 0; this.score = 0; this.maxChain = 0; this.lastChain = 0;
      this.recentChainTimer = 0; this.pressureSent = 0; this.pressureReceived = 0;
      this.attackCount = 0; this.defenseCount = 0; this.flow = 0; this.flowTimer = 0;
      this.moveRepeat = { dir: 0, held: 0, repeat: 0 };
      this.board.reset();
      this.ai?.reset();
    }
    ...
    update(dt, input) {
      ...
      if (this.flowTimer <= 0) this.flow = Math.max(0, this.flow - dt * .35);
      this.displayCp += (this.cp - this.displayCp) * (1 - Math.pow(.0001, dt));
    ...
    updateHuman(dt, input) {
      ...
      if (input.consume('rotateCW')) {
        if (!this.board.rotate(1)) this.game.bus.emit('invalid', { player: this, action: 'rotate' });
      }
      ...
      if (!dir) {
        ...
        if (this.moveRepeat.held > .135) {
          this.moveRepeat.repeat += dt;
          while (this.moveRepeat.repeat > .045) {
            this.moveRepeat.repeat -= .045;
    ...
    onClearStart(clear, chain, counter) {
      this.maxChain = Math.max(this.maxChain, chain);
      this.flow = clamp(this.flow + .14 + chain * .055, 0, 1);
      this.flowTimer = 3.1;
      this.game.hitStop = Math.max(this.game.hitStop, this.game.settings.reducedMotion ? 0 : Math.min(.105, .022 + chain * .013 + clear.pulse * .025));
    }
    ...
    onClearResolved(clear, chain, counter) {
      ...
      const multiplier = 1 + (chain - 1) * .55 + (counter ? .18 : 0);
      this.score += Math.round((clear.regular * 120 + clear.pulse * 2800 + chainBonus * 8) * multiplier);
      this.game.bus.emit('energy', { player: this, amount: energy, cp: this.cp, clear, chain, counter });
      if (this.game.tutorial?.active && this.isHuman && clear.pulse > 0) this.game.tutorialOnPulseClear();
    }
    ...
    castAttack() {
      ...
      this.cp = 0;
      const activeChain = this.board.resolution?.chain || (this.recentChainTimer > 0 ? this.lastChain : 0);
  ...
  class AIController {
    ...
    reset() {
      this.target = null; this.activeId = -1; this.thinkTimer = 0; this.stepTimer = 0; this.castCooldown = 0; this.alignedTimer = 0;
    }
    ...
    update(dt) {
      ...
      if (board.active.id !== this.activeId) {
        ...
        this.target = null;
        this.thinkTimer = this.difficulty.decisionDelay / 1000 + this.rng.next() * .12;
        this.alignedTimer = 0;
      }
      ...
      this.stepTimer = this.difficulty.aiStep / 1000 * (.85 + this.rng.next() * .3);
      ...
      if (this.alignedTimer >= this.difficulty.decisionDelay / 1000 * .65) board.hardDrop(false);
    }
    ...
    updateCasting() {
      ...
      if (incoming > 0 && (boardDanger >= this.profile.defendDanger || incoming >= 3 || this.player.cp >= 500)) {
        if (this.player.castDefense()) this.castCooldown = .55 + this.rng.next() * .6;
        return;
      }
      const opponent = this.game.otherPlayer(this.player);
      let threshold = this.profile.attackAt;
      if (this.profile.id === 'mirror') {
        threshold = opponent.board.dangerRatio() > .55 ? 100 : opponent.cp > this.player.cp ? 280 : 170;
      }
      if (opponent.board.dangerRatio() > .72) threshold = Math.min(threshold, 100);
      if (this.player.cp >= threshold && (this.rng.next() < .035 * this.profile.attackBias || this.player.cp >= threshold + 90)) {
        if (this.player.castAttack()) this.castCooldown = .45 + this.rng.next() * .55;
      }
    }
    ...
    choosePlacement() {
      ...
      for (let rot = 0; rot < 4; rot++) {
        for (let x = -1; x <= CONFIG.COLS; x++) {
          ...
          for (const pos of placement.cells) grid[pos.y][pos.x] = { ...cellTemplate(pos.cell), id: 1 };
          ...
          if (placement.cells.some(c => c.cell.kind === 'pulse')) {
            for (const pc of placement.cells.filter(c => c.cell.kind === 'pulse')) {
              for (const [dx,dy] of DIRS) {
                ...
                if(inside(nx,ny)&&sim.grid[ny][nx]?.kind==='normal') score += 15 * p.pulseWeight;
              }
            }
          }
      ...
      if (!candidates.length) return { x: CONFIG.CENTER_COL, rot: active.rot, y: 0, score: -Infinity };
    ...
    dropPlacement(grid, active, x, rot) {
      ...
      const collisionAt = y => {
        for (let i=0;i<2;i++) {
          const px=x+offsets[i][0], py=y+offsets[i][1];
          if(px<0||px>=CONFIG.COLS||py>=CONFIG.ROWS||py<0||grid[py][px]) return true;
        }
        return false;
      };
  ...
  class GameSession {
    constructor(options) {
      ...
      this.cpu = new Player(this, {
        id:'cpu', name:this.profile.label, isHuman:false, rng:cpuRng,
        spawnDisabled: this.mode === 'lab',
        profile:this.profile,
        ai: this.mode === 'lab' ? null : { profile:this.profile, difficulty:this.difficultyConfig, rng:cpuRng.fork(0x987) }
      });
      this.players = [this.player, this.cpu];
      this.tutorial = options.tutorial ? { active:true, stage:0, timer:0, sentDefenseLesson:false, complete:false } : null;
    ...
    seedLabTarget() {
      ...
      for (let y = CONFIG.ROWS - 1; y >= CONFIG.ROWS - 5; y--) {
        for (let x = 0; x < CONFIG.COLS; x++) {
          if (this.rng.chance(.18)) continue;
    ...
    otherPlayer(player) { return player === this.player ? this.cpu : this.player; }
    ...
    update(dt, input) {
      ...
      this.globalIntensity = clamp(Math.max(this.player.flow, this.cpu.flow, this.player.board.dangerRatio(), this.cpu.board.dangerRatio() * .8), 0, 1);
    }
    ...
    launchAttack(from, target, lines, spent, chain = 0) {
      const projectile = {
        id: globalProjectileId++, from, target, lines, originalLines:lines, spent, chain,
        elapsed:0, duration:CONFIG.ATTACK_TRAVEL_MS + lines * 25, canceled:0
      };
    ...
    resolveDefense(player, spent) {
      ...
      const incoming = this.projectiles.filter(p => p.target === player && p.lines > 0).sort((a,b) => (a.duration-a.elapsed)-(b.duration-b.elapsed));
      let canceled = 0;
      for (const p of incoming) {
        if (units <= 0) break;
        const used = Math.min(units, p.lines);
        p.lines -= used; p.canceled += used; units -= used; canceled += used;
      ...
      if (units > 0 && player.board.pendingLines > 0) {
        ...
        player.board.pendingLines -= used; units -= used; canceled += used;
      }
      const purgeCount = units * 4;
      const purged = purgeCount ? player.board.purgeCells(purgeCount) : 0;
      player.board.stabilizeTimer = Math.max(player.board.stabilizeTimer, .4);
      this.bus.emit('defense', { player, spent, canceled, purged, units:Math.max(1,Math.floor(spent/90)) });
    }
    ...
    endByBlitzScore() {
      ...
      const pv = value(this.player), cv = value(this.cpu);
    ...
    tutorialInput(action) {
      ...
      if (!t?.active) return;
      if (t.stage === 0 && (action === 'left' || action === 'right')) { t.stage = 1; t.timer = 0; this.bus.emit('tutorialStage',{stage:1}); }
      else if (t.stage === 1 && (action === 'rotateCW' || action === 'rotateCCW')) { t.stage = 2; t.timer = 0; this.bus.emit('tutorialStage',{stage:2}); }
      else if (t.stage === 2 && action === 'hardDrop') { t.stage = 3; t.timer = 0; this.bus.emit('tutorialStage',{stage:3}); }
    }
    ...
    tutorialOnDefense() {
      ...
      t.active = false; t.complete = true; t.stage = 7;
    ...
    snapshot() {
      const packPlayer = p => ({
        id:p.id, cp:p.cp, score:p.score, maxChain:p.maxChain, lastChain:p.lastChain,
        pressureSent:p.pressureSent, pressureReceived:p.pressureReceived,
        danger:p.board.dangerRatio(), pending:p.board.pendingLines,
        grid:p.board.grid.map(row => row.map(c => c ? {kind:c.kind,color:c.color} : null)),
        active:p.board.active ? {x:p.board.active.x,y:p.board.active.y,rot:p.board.active.rot,a:{...p.board.active.a},b:{...p.board.active.b}} : null
      });
      return { version:CONFIG.VERSION, seed:this.seed, mode:this.mode, state:this.state, elapsed:this.elapsed, timeLeft:this.timeLeft, player:packPlayer(this.player), cpu:packPlayer(this.cpu), projectiles:this.projectiles.map(p=>({from:p.from.id,target:p.target.id,lines:p.lines,elapsed:p.elapsed,duration:p.duration})), tutorial:this.tutorial?{...this.tutorial}:null };
    }
  }

  PLO.logic = { findClearData, collapseGrid, simulateResolution, gridMetrics, groupPotential, createEmptyGrid };
