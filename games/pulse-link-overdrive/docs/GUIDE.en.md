# PULSE LINK // OVERDRIVE Guide

[Play online](https://hitsuki-ban.github.io/PulseLinkOverdrive/) · [简体中文](GUIDE.zh-CN.md) · [日本語](GUIDE.ja.md)

## Three modes

- **DUEL**: A standard match against the CPU. Either side loses when its central spawn column reaches the top.
- **BLITZ**: A 90-second match. If nobody tops out first, score, pressure sent, and board danger decide the result when time expires.
- **LAB**: A non-attacking practice target. Its board resets when it tops out; your own central-column top-out still ends the session.

## Core rules

1. Drop a rotatable pair of pieces at a time.
2. Three or more pieces of one color clear when connected up, down, left, or right. A new clear after gravity creates a chain.
3. A white pulse piece caught next to a clearing group grants 100 energy.
4. At 100 energy you may attack or defend. Either choice spends all your energy.
5. Attacks lift ordinary colored rows into the opponent's board, so they can accidentally complete an opponent's chain. Defense cancels incoming rows first, then uses any strength left to remove high pieces.
6. You lose only when the central spawn column enters the hidden area. The side columns may be stacked higher.
