...
- Issue #52 admits the rebuilt guest through the snapshot status `production-admitted`; the archive, standalone HTML, local launchers, supplied previews, baseline tools, screenshots, and performance evidence remain excluded from production inputs.
...
## Before-refactor performance evidence

Issue #50's bounded diagnostic aggregate remains immutable at `performance/before-evidence.json`; it
is historical evidence, not a release budget. Issue #52 replaces the live
`performance/neon-overdrive.perf.spec.ts` with the admitted Hub + Guest release matrix and records
that separate result in `performance/after-evidence.json`; see
`performance/REFERENCE_PERFORMANCE.md` for its fixed procedure and budgets.
