...
The hidden measurement is intentionally named `hidden-frozen`. Chromium automation does not naturally change `document.hidden` when `Page.setWebLifecycleState` freezes a target, so the harness explicitly controls `document.hidden`, dispatches a test-owned `visibilitychange` event through the source game's real listener, and then applies the real CDP `frozen` lifecycle. The evidence records both mechanisms and must not be described as a naturally backgrounded browser tab.

| Scenario      | RAF evidence                                    | CDP Task / Script / Layout | Activity and peaks                                                                                                           |
| ------------- | ----------------------------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Normal        | 4 intervals; p95/max 466.7 ms; all 4 over 20 ms | 20.82 / 10.66 / 1.91 ms    | 25 playing ticks, 117 HUD mutations, 1 audio callback; peaks 3 enemies / 2 enemy bullets / 39 player bullets / 102 particles |
| 4× CPU        | 1 interval at 66.6 ms, over 20 ms               | 81.95 / 46.78 / 4.74 ms    | 8 playing ticks, 36 HUD mutations, 1 audio callback                                                                          |
| Dense         | 1 interval at 83.34 ms, over 20 ms              | 14.69 / 7.39 / 0.47 ms     | 10 playing ticks, 45 HUD mutations, 1 audio callback; peaks 422 enemy bullets and 395 particles                              |
| Paused        | 5 intervals; p95/max 333.4 ms; all 5 over 20 ms | 13.70 / 5.00 / 0 ms        | 29 update calls but 0 playing ticks, 0 HUD mutations, and 2 audio callbacks                                                  |
| Hidden-frozen | 0 RAF intervals                                 | 23.18 / 1.41 / 0 ms        | 0 update calls, playing ticks, and HUD mutations; 35 audio-scheduler callbacks; final game state paused                      |
